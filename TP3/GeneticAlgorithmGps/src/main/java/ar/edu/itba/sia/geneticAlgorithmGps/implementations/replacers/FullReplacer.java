...
public class FullReplacer implements Replacer {
    ...
    public FullReplacer(List<Selector> selectionAlgorithms, int quant1, int quant2){
    ...
    public List<Chromosome> replace(List<Chromosome> population, List<Chromosome> children, List<Chromosome> selected) {
        ...
        newGen.addAll(algos.get(1).select(selection, (int) Math.floor( population.size() * percent2 ) ));
        return newGen;
    }
}
